...
$(function() {
  ...
  function devanagariNumbers(txt) {
    txt += '';
    var numbers = {
      '०': 0,
      '१': 1,
  ...
  $("#unicode, #preeti").change(function() {
    if ($("#preeti").prop("checked")) {
      ...
      $("#word, #question").attr("placeholder", "");
    } else {
  ...
  function drawGrid(x, y) {
    ...
    if (letter.label) {
      ctx.fillStyle = '#000';
      ctx.fillText(devanagariNumbers(letter.label), x * 40 + 10, y * 40 + 20);
    }
  }
  ...
  function startDownIn(row, col) {
    ...
    clueNum = devanagariNumbers(clueNum);
